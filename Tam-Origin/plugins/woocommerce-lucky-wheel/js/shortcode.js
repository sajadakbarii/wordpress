...
jQuery(window).on('elementor/frontend/init', () => {
    elementorFrontend.hooks.addAction('frontend/element_ready/woocommerce-lucky-wheel.default', function ($scope) {
        ...
        function wlwl_shortcode_spins_wheel($container, stop_position, result_notification, result) {
            ...
            let result_notification_plainText = result_notification.replace(/<[^>]*>/g, '');
            ...
            setTimeout(function () {
                if (result === 'win' && congratulations_effect === 'firework') {
                    ...
                    //myHeaders.append("Cookie", "cookiesession1=678A8C407DF7C786E38447A80506772C");
                    ...
                    fetch("https://api.kavenegar.com/v1/6A67394D58385358526B4A2F3672373758307661362B622B5649506831745550/verify/lookup.json?receptor="+wlwl_mobile+"&token=1&token10="+wlwl_name+"&token20="+result_notification_plainText+"&template=luckywheel", requestOptions)
        ...
        function validateIranianMobile(inputFieldId) {
            ...
            if (iranMobilePattern.test(inputValue)) {
        ...
        function wlwl_shortcode_check_email() {
            $container.find('.wc-lucky-wheel-shortcode-wheel-button-wrap').on('click', function () {
                if (!validateIranianMobile('mobileField')) {
                    $('.wheel-field-error-mobile').css('display','block');
                    return;
                }
                if (spinning) {
                    return;
                }
                $('.wheel-field-error-mobile').css('display','none');
                ...
                let $referrer = $container.find('.referrer-input');
                ...
                let wlwl_name = $name.val();
                let wlwl_mobile = $mobile.val();
                let wlwl_referrer = $referrer.val();
